...
class SarjIstasyonuServisi {
  static const String _nominatimUrl =
      'https://nominatim.openstreetmap.org/search';

  Future<List<Hizmet>> sarjIstasyonlariniGetir({
    required String il,
    ...
    try {
      ...
      throw Exception(
        'Şarj istasyonu verisi alınırken hata oluştu: $e',
      );
    }
  }
  ...
  Future<List<Hizmet>> getir({
    ...
    return sarjIstasyonlariniGetir(
      il: il,
      ilce: ilce,
    );
  }
...
[out:json][timeout:45];
(
  nwr["amenity"="charging_station"]$alan;
  nwr["amenity"="fuel"]["fuel:electricity"]$alan;
  nwr["man_made"="charging_station"]$alan;
  nwr["motorcycle:charging_station"]$alan;
  nwr["name"~"şarj|sarj|charging station|ev charging|electric vehicle",i]$alan;
);
out body center;
''';
  ...
  Future<List<Hizmet>> _ilceAlanSorgula({
    ...
    final String ilRegex = _overpassRegex(il);
    final String ilceRegex = _overpassRegex(ilce);
...
(
  nwr["amenity"="charging_station"](area.ilceArea);
  nwr["amenity"="fuel"]["fuel:electricity"](area.ilceArea);
  nwr["man_made"="charging_station"](area.ilceArea);
  nwr["motorcycle:charging_station"](area.ilceArea);
  nwr["name"~"şarj|sarj|charging station|ev charging|electric vehicle",i](area.ilceArea);
);
...
''';
  ...
  Future<List<Hizmet>> _sorguyuCalistir({
    ...
    final dynamic elements = data['elements'];
    ...
    for (final dynamic element in elements) {
      ...
      final dynamic tagsDynamic = element['tags'];
      ...
      final String id =
          'sarj_${element['type']}_${element['id']}';

      if (!bulunanIdler.add(id)) {
        continue;
      }

      final String isim = _isimGetir(tags);

      final String adres =
          _adresOlustur(tags, il, ilce);
      ...
      final String durum =
          _durumGetir(tags['opening_hours']);

      sonuc.add(
        Hizmet(
          id: id,
          isim: isim,
          kategori: 'Elektrikli Şarj İstasyonu',
          adres: adres,
          telefon: telefon,
          durum: durum,
          mesafe: '-',
